...
function renderMobileStickyActionBar(html){
  if(!isPhoneViewport() || !html) return '';
  return `<div class="mobile-sticky-actions">${html}</div>`;
}
function renderMobileActionChip(label, value, tone){
  var toneClass = tone ? ` mobile-meta-value-${tone}` : '';
  return `<div class="mobile-meta-block"><div class="mobile-meta-label">${label}</div><div class="mobile-meta-value${toneClass}">${value}</div></div>`;
}
...
function initThreeJS(){
  if(typeof THREE === 'undefined'){
    if(typeof ensureThreeJS === 'function') ensureThreeJS().then(function(){ initThreeJS(); });
    return;
  }
...
function openMo(html){
  ...
  var mo=document.getElementById('mo');
  var box=document.getElementById('mo-box');
  if(!body || !mo) return;
  _moReturnFocus = document.activeElement;
  body.innerHTML=html;
  syncModalViewportMode();
  mo.classList.add('open');
  mo.setAttribute('role','dialog');
  mo.setAttribute('aria-modal','true');
  // Set aria-label from first .mo-title if present
  var title = body.querySelector('.mo-title');
  if(title) mo.setAttribute('aria-label', title.textContent);
  else mo.removeAttribute('aria-label');
  document.body.classList.add('modal-open');
  if(box) box.scrollTop=0;
  // Focus first focusable element inside modal
  setTimeout(function(){
    var focusable = body.querySelector('input:not([type=hidden]),select,textarea,button,[tabindex]');
...
function closeMo(){
  ...
  if(mo){
    ...
    mo.removeAttribute('role');
    mo.removeAttribute('aria-modal');
    mo.removeAttribute('aria-label');
  }
  document.body.classList.remove('modal-open');
  // Restore focus to trigger element
  if(_moReturnFocus && typeof _moReturnFocus.focus === 'function'){
    try{ _moReturnFocus.focus(); }catch(e){}
    _moReturnFocus = null;
  }
}
document.addEventListener('keydown',function(e){
  ...
  if(e.key==='Tab'){
    ...
    var focusable = mo.querySelectorAll('input:not([type=hidden]),select,textarea,button:not([disabled]),[tabindex]:not([tabindex="-1"])');
...
window.addEventListener('resize',syncModalViewportMode);

// Styled confirmation modal — replaces native confirm() dialogs
// Usage: openConfirmModal({ title, message, confirmLabel, danger, onConfirm })
function openConfirmModal(opts){
  var o = opts || {};
  var title = o.title || (LANG==='es' ? 'Confirmar' : 'Confirm');
  var msg = o.message || '';
  var confirmLabel = o.confirmLabel || (LANG==='es' ? 'Eliminar' : 'Delete');
  var cancelLabel = LANG==='es' ? 'Cancelar' : 'Cancel';
  ...
  var btn = document.getElementById('_confirm-action-btn');
  if(btn && o.onConfirm) btn.onclick = function(){ closeMo(); o.onConfirm(); };
}
...
document.addEventListener('touchend', function(e){
  ...
  var dy = e.changedTouches[0].clientY - _lbTouchStartY;
